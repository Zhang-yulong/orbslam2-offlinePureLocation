...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap, shared_ptr<PointCloudMapping> pPointCloud, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, const bool bShowPointCloud, const bool bLocalizationMode):
    mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
    mpPointCloudMapping( pPointCloud ),mbTrackingShowPointCloud( bShowPointCloud ), mbLocalizationMode(bLocalizationMode),
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)// 没有成功初始化的前一个状态就是NO_IMAGES_YET
        //mpIniORBextractor 与 mpORBextractorLeft 相比提取特征点多一倍
        //这里Frame构造函数中，有ExtractORB函数进行提取特征点
...
void Tracking::Track()
{
    ...
    std::cout<<"Tracking.cc :: Track, 获得当前地图关键帧数目： "<<mpMap->GetMaxKFid()<<std::endl;
    ...
    if(mState == NOT_INITIALIZED && mpMap->GetMaxKFid() == 0)
    {
        ...
        mpFrameDrawer->Update(this);  //mpFrameDrawer对象在System类中被创建，该函数需要传入Tracking的类对象，在Tracking里被调用，使用this
        // cout<<"Drawer更新"<<endl;
        if(mState!=OK)
            {return;}
    ...
    {
       if(mSensor==System::STEREO || mSensor==System::RGBD){
            cout<<"存在地图, 双目和RGBD初始化"<<endl;
            
        }
        else{
            cout<<"存在地图, 进入单目初始化"<<endl;
        ...
        if(mState!=OK){
            cout<<"存在地图, mState!=OK, return"<<endl;
            mState = NOT_INITIALIZED;
            return;
        }

    }
    else// 步骤2：初始化成功，相机位姿跟踪
    {
        cout<<"初始化成功，相机位姿跟踪，局部地图"<<endl;
        ...
        // mbOnlyTracking等于false表示正常VO模式（有地图更新），mbOnlyTracking等于true表示用户手动选择定位模式(创建对象的时候定为false)
        if(!mbOnlyTracking) //开始SLAM模式
        {
        ...
        {
            cout<<"----Tracking.cc :: 纯定位模式, 该部分进行跟踪tracking工作, 局部地图先不管"<<endl;
            ...
            if(mState==LOST)
            {   
                cout<<"Tracking.cc (纯定位模式) :: mState==LOST, 跟踪失败, Relocalization()"<<endl;
                bOK = Relocalization();
            }
            else
            {
                // mbVO是mbOnlyTracking为true时的才有的一个变量（仅定位）
                // mbVO为false表示此帧匹配了很多的MapPoints，跟踪很正常，
                // mbVO为true表明此帧匹配了很少的MapPoints，少于10个，要跪的节奏
                if(!mbVO) //（ ---系统构造默认是false---- ）
                {
                    // In last frame we tracked enough MapPoints in the map
                    // mbVO为0则表明此帧匹配了很多的3D map点，非常好
                    cout<<"Tracking.cc (纯定位模式) :: mbVO == false, 该帧匹配了很多地图点"<<endl;
                    if(!mVelocity.empty())
                    {
                        cout<<"Tracking.cc (纯定位模式) :: mVelocity not empty, TrackWithMotionModel()"<<endl;
                        bOK = TrackWithMotionModel();
                        // 这个地方是不是应该加上：
                        // if(!bOK)
                        //    bOK = TrackReferenceKeyFrame();
                    ...
                    {   
                        cout<<"Tracking.cc (纯定位模式) :: mVelocity is empty, TrackReferenceKeyFrame()"<<endl;
                        bOK = TrackReferenceKeyFrame();
                    }
                }
                else
                {
                    cout<<"Tracking.cc (纯定位模式) :: mbVO == true, 该帧匹配了地图点少于10, 接下来做跟踪和定位"<<endl;
                    ...
                    if(bOKMM && !bOKReloc)
                    {
                        cout<<"Tracking.cc (纯定位模式) :: bOKMM && !bOKReloc; 跟踪成功，重定位失败"<<endl;
                        // 这三行没啥用？
                    ...
                    {
                        mbVO = false;
                        cout<<"Tracking.cc (纯定位模式) :: bOKReloc == true; 更相信重定位结果, mbVO重新标记为false, 有更多匹配的地图点 "<<endl;
                    }

                    bOK = bOKReloc || bOKMM;

                    cout<<"Tracking.cc (纯定位模式) :: 跟踪部分的结果: 重定位bOKReloc = "<<bOKReloc<<", 跟踪bOKMM = "<<bOKMM<<endl;
                }
            }
        }
        ...
        else  //仅定位
        {
            ...
            if(bOK && !mbVO){
                bOK = TrackLocalMap();
                cout<<"----Tracking.cc (纯定位模式) :: 局部地图跟踪TrackLocalMap()结果 = "<<bOK<<endl;
        ...
        if(bOK){
            mState = OK;
            cout<<"Tracking.cc (纯定位模式) :: 现在 mState = OK "<<endl;
        }
        else{
            mState = LOST;
            cout<<"Tracking.cc (纯定位模式) :: 现在 mState = LOST "<<endl;
        }
        // Update drawer
        mpFrameDrawer->Update(this);
        ...
        if(bOK)
        {
            ...
            if(NeedNewKeyFrame())  
                CreateNewKeyFrame(); //该函数里面生成点云
            idk++;//上个版本木有（关键帧索引）（点云模块）
    ...
    {
        cout<<"Tracking.cc::Track() : 整个跟踪失败 "<<endl;
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            //用于将指定的值赋给一个给定范围内的所有元素，常用于数据结构初始化
    ...
    {
        // cout<<"初始帧ID: "<< mInitialFrame.mnId<<" ,点数："<<mInitialFrame.mvKeys.size()<<" ,上一帧ID: "<<mLastFrame.mnId<<" ,特征点数: "<<(int)mLastFrame.mvKeys.size()<<" ,当前帧ID: "<<mCurrentFrame.mnId<<" ,特征点数： "<<(int)mCurrentFrame.mvKeys.size()<<endl;
        cout<<"初始帧ID: "<< mInitialFrame.mnId<<" ,点数："<<mInitialFrame.mvKeys.size()<<" ,当前帧ID: "<<mCurrentFrame.mnId<<" ,特征点数： "<<(int)mCurrentFrame.mvKeys.size()<<endl;
        // Try to initialize
        ...
        if(nmatches<80)  //正常100
        {
            delete mpInitializer;
            mpInitializer = static_cast<Initializer*>(NULL);
            cout<<"太少匹配点对数："<<nmatches<<endl;
            return;
        }
...
cout<<"单目三角化"<<",接下来"<<endl;    
    ...
    // Bundle Adjustment
    cout << "New Map created with " << mpMap->MapPointsInMap() << " points" <<"开始关闭Optimizer::GlobalBundleAdjustemnt(mpMap,20)"<< endl;
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if(mbOnlyTracking){
        std::cout<<"进入NeedNewKeyFrame()，由于仅定位模式，不会创建关键帧"<<std::endl;
        return false;
    }
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if(mSensor!=System::MONOCULAR)
    {
        ...
        vector<pair<float,int> > vDepthIdx;
        vDepthIdx.reserve(mCurrentFrame.N);  //更改容器大小
        for(int i=0; i<mCurrentFrame.N; i++)
        {
            float z = mCurrentFrame.mvDepth[i];
            if(z>0)
    ...
    mpLocalMapper->InsertKeyFrame(pKF);  //往局部地图的队列中加入关键帧

    mpLocalMapper->SetNotStop(false);
    vector<KeyFrame*> vpKFs = mpMap->GetAllKeyFrames(); // 取出所有的关键帧
    
    if(mSensor == System::RGBD && mbTrackingShowPointCloud)
    {   
        // insert Key Frame into point cloud viewer
        //生成关键帧在相机坐标系下的点云，再将此点云变换到世界坐标系保存为全局点云地图。
...
void Tracking::MonocularInitializationWithMap(){
    ...
    if(mCurrentFrame.N > 500){
        ...
        bool bOK;
        bOK = Relocalization(); // BOW搜索，PnP求解位姿
        cout<<"存在地图(初始化过程), Relocalization结果: "<<bOK<<endl;

        //-----下面就和Tracking::Track()的流程一样-------
        ...
        if(!mbOnlyTracking){
            ...
            if(bOK && !mbVO){
                bOK = TrackLocalMap();  //局部地图跟踪 
                cout<<"存在地图(初始化过程), TrackLocalMap结果: "<<bOK<<endl;
            }
        }

        if(bOK){
            cout<<"存在地图(初始化过程), 现在 mState = OK "<<endl;
            mState = OK;
        }    
        else{
            cout<<"存在地图(初始化过程), 现在 mState = LOST "<<endl;
            mState = LOST;
        }
        // Update drawer
        mpFrameDrawer->Update(this);
        ...
        if(bOK){
            ...
            // Check if we need to insert a new keyframe
            // 步骤2.6：检测并插入关键帧，对于双目会产生新的MapPoints
            //因为mbOnlyTracking为true，实际这里并不会生成新关键帧
            if(NeedNewKeyFrame())  
                CreateNewKeyFrame(); 
        ...
        if(mState==LOST)
        {
            ...
            {
                //查看
                cout << "存在地图(初始化过程), Track lost soon after initialisation, reseting..." << endl;
                mpSystem->Reset();
                return;
            }
        }
